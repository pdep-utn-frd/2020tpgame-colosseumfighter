...
object prota{
	...
	method earnXp(){
		...
		else {  game.removeTickEvent("Ganar") inicioLobby.inicio()}
	}
	method respawn(){
		self.nextLvlAt(40)
		self.fuerza(5)
	...
	method pelear(enemigo){
		...
		if  ((atPer) > (30+peso**(1/2))) {
			...
			game.onTick(4100,"restriccion de ataque",{=>turno.atacando(false) game.removeTickEvent("restriccion de ataque")})
...
//const enemigo1 = new Gladiador(arma=creator.arma(),clave=creator.clave(),num=creator.num())
