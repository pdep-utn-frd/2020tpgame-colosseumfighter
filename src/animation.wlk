...
//--------------------------------------------------Caminar+Atacar-Derecha----------------------------------------------------------//		
	method attackActionR(charact, enem) {
		...
		game.onTick(1000, "animationAttackR", {=>
			self.atacar(charact)
			game.removeTickEvent("animationAttackR")
		})
		if (enem.vida() <= 0) {
			game.onTick(1695, "animacionDying", {=>
				self.dying(enem)
				game.removeTickEvent("animacionDying")
			})
		} else {
			game.onTick(1695, "animacionGolpeR", {=>
				self.hurt(enem)
				game.removeTickEvent("animacionGolpeR")
			})
		}
		game.onTick(2410, "animacionBackwardR", {=>
			self.walkingL(charact)
			game.removeTickEvent("animacionBackwardR")
		})
	}
...
//--------------------------------------------------Caminar+Atacar-Izquierda--------------------------------------------------------//
	method attackActionL(charact, enem) {
		self.walkingL(charact)
		game.onTick(1000, "animationAttackL", {=>
			self.atacar(charact)
			game.removeTickEvent("animationAttackL")
		})
		if (enem.vida() <= 0) {
			game.onTick(1695, "animacionDying", {=>
				self.dying(enem)
			...
			game.onTick(1695, "animacionGolpeL", {=>
				self.hurt(enem)
				game.removeTickEvent("animacionGolpeL")
			})
		}
		game.onTick(2410, "animacionBackwardL", {=>
			self.walkingR(charact)
			game.removeTickEvent("animacionBackwardL")
		})
	}
